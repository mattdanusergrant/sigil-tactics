...
- **Any unit may cast any card.** The active unit casts; the card's owner (the hero whose suite contributed it) provides the *assist* from off-field. Owner must still be **alive** to assist — killing them dead-sources their 6 cards. Range and self-target effects always resolve on the caster, not the owner.
- **Sigils** (`card.hero === "sigil"`, 10 cards shared across every deck) have no owner. Any unit may cast any sigil. Casting a sigil consumes **both the move and the action** for the turn — the unit's turn ends immediately on resolution. Sigils are **removed from the game** on use (no discard pile; they never re-enter the deck or hand).
